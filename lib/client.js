...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var RECONNECT_TIMEOUT_MS = 15000 // 新进程就绪后等待 DSH 自动重连的兜底时长
    ...
    function RestartButton(props) {
        ...
        fetch(HEALTH_URL, { cache: 'no-store' })
          ...
          .then(function (health) {
            if (Number.isInteger(health && health.pid) && health.pid !== currentPid.current) {
              // 新进程已就绪。不要整页刷新 —— 那会清空客户端 current 会话、让
              // DSH 回退到"最近工作区新建会话"（表现为新开标签）。改为保持页面，
              // 靠 DSH 客户端的自动重连（runtime 会 emit connection/reset）恢复界面。
              stopPolling()
              waitForReconnect()
              return
            }
      ...
      // 新进程已就绪：等待 DSH 自动重连完成后恢复按钮；带超时兜底，避免卡在"重启中"。
      function waitForReconnect() {
        // 若连接已恢复（health 就绪时 WebSocket 往往已重连），直接恢复。
        var desc = null
        try { desc = clientCtx && clientCtx.connection && clientCtx.connection.hostDescription.getSnapshot() } catch (e) { desc = null }
        if (desc) {
          setPhase('idle')
          return
        }
        ...
        var off = null
        reconnectTimer = setTimeout(function () { // 兜底：等待过久则恢复，不卡死
          if (off) off()
          setPhase('idle')
        }, RECONNECT_TIMEOUT_MS)
        off = clientCtx.on('connection/reset', function () {

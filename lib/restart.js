 ...
 *      process — preserving `--profile` / `--patch` overlays and inner app
 *      flags, plus `--no-open` (so the restarted harness does not pop a
 *      browser);
...
export function createRestarter({ spawnImpl = spawn, exitImpl = (code) => process.exit(code), relaunchScript = RELAUNCH_SCRIPT, argv = process.argv } = {}) {
  ...
  function requestRestart({ port, reason } = {}) {
    ...
    restarting = true
    try {
      // 重启命令总是确保带 --no-open：重启时浏览器已在用户面前（页面由插件自动
      // 恢复），没必要再自动弹一个；原命令已带 --no-open 则保持不变。
      const args = argv.slice(2)
      if (!args.includes('--no-open')) args.push('--no-open')
      const child = spawnImpl(
        process.execPath,
        [relaunchScript, process.argv[1], ...args],
        {
          stdio: 'inherit',
          env: {
            ...process.env,

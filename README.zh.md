...
- ♻️ **完整进程重启**：结束当前 `dsh web` 进程，以**完全相同的命令行**重新拉起（保留 `--profile` / `--patch` 及所有内层参数，并总是确保带 `--no-open` 不弹浏览器）
- 🛡️ **启动守卫（无守护进程）**：新进程由守卫在「旧进程已退出 + 端口已释放」后才启动，彻底避免端口占用竞态（DSH 的 webserver 启动即监听且无重试）
- 🔄 **页面不刷新、不新建标签**：重启期间按钮显示「重启中…」，页面保持不动，DSH 客户端 WebSocket 自动重连（`connection/reset`）在**原会话标签页**恢复界面（会话由 DSH 持久化保留）
...
### Web 按钮
侧边栏底部点击「重启 DSH」即可。按钮变「重启中…」并自动轮询；**页面不刷新** —— DSH 的 WebSocket 自动重连在**同一会话标签页**恢复界面（不新建标签）。若 30 秒内新进程未就绪，会显示错误提示（请查看运行 dsh 的终端日志）。
